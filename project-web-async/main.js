...
const getPhotos = async (
  ...
  try {
    const data = await fetch(
      `https://api.unsplash.com/search/photos?per_page=${photoNum}&query=${keyword}&orientation=${photoOrientation}&order_by=${photoRelevant}&page=${page}&client_id=${
...
const printPhotos = (photos) => {
  ...
  if (photos.length === 0) {
    ...
    container.innerHTML = "";
    message.textContent =
      "No se encontraron resultados, por favor inicia una nueva búsqueda...";
  } else {
    noDisplay();
...
document.querySelector("#searchBtn").addEventListener("click", () => {
  ...
  const photoOrientationValue =
    document.querySelector("#orientationInput").value;
  const photoRelevantValueValue =
    document.querySelector("#relevantInput").value;
  currentPage = 1;
  getPhotos(
    keywordValue,
    photoNumValue,
    photoOrientationValue,
    photoRelevantValueValue,
    currentPage
  );
});
...
document.querySelector("#nextPage").addEventListener("click", () => {
  const totalPages = parseInt(
    document.querySelector("#pageInfo").textContent.split("de ")[1],
    10
  );
